...
def normalize_(image, MIN_B=-1024, MAX_B=3072):#-1024,3072
...
def compute_SSIM(img1, img2, data_range, window_size=11, channel=1, size_average=True):
    ...
    if len(img1.shape) == 2:
        # shape_ = img1.shape[-1]
        shape0, shape1 = img1.shape[-2], img1.shape[-1]
        # img1 =np.expand_dims(img1, axis=(0, 1))
        img1 = img1.reshape(1,1,shape0 ,shape1)
        img2 = img2.reshape(1,1,shape0 ,shape1)
