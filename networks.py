...
class SobelConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.sobel_weight = nn.Parameter(torch.full(size=(out_channels, int(in_channels / groups), kernel_size, kernel_size)
                                                    , fill_value=0.1), requires_grad=False)
...
def window_partition(x, window_size):
    ...
    # tensor.permute(a,b,c,d)将tensor的维度换位,可以对任意高维矩阵进行转置。
    # torch.transpose(Tensor, a,b)transpose只能操作两个维度进行转置。
    # 调用contiguous()时，会强制拷贝一份tensor，让它的布局和从头创建的一模一样，但是两个tensor完全没有联系。（即深拷贝）
    return windows
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # torch.stack()和torch.cat()均用于拼接，stack扩维，cat不扩维
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        self.window_size = window_size
        # self.conv_sobel = SobelConv2d(in_chans, embed_dim, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            x = layer(x, x_size)
            # print(x.shape) #[20,400,60]

        x = self.norm(x)  # B L C
        x = self.patch_unembed(x, x_size)
        # print(x.shape) #[20.60.20.20]
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # x_sobel=self.conv_sobel(x)
            # x_first=x
            # print(x_first.shape) #[20,60,20,20]
            # res=self.forward_features(x_first)
            # print(res.shape)
            # res=self.conv_after_body(res)+x_first
            # print(res.shape)
            # res = self.conv_after_body(self.forward_features(x_first)) + x_first + x_sobel
            res = self.conv_after_body(self.forward_features(x)) + x
            # print(res.shape)
            # res_new=self.conv_last(res)
            # x = x + res
    ...
    #     return loss
    #
    # def loss(self,x,y,perceptual=True,return_p=True):
    #     r_loss=self.p_criterion(x,y)
    #     if perceptual:
    #         p_loss=self.p_loss(x,y)
    #         loss=r_loss+(0.3*p_loss)
    #     else:
    #         p_loss=None
    #         loss=r_loss
    #     return(loss,p_loss) if return_p else loss

def swinIR_1_denoising(*kwargs):
    model=SwinIR(upscale=1, img_size=(64, 64),in_chans=1,patch_size=1,
                   window_size=8, img_range=1., depths=[4, 4, 4],
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='')
    return model

def swinIR_2_denoising(*kwargs):
    model=SwinIR(upscale=1, img_size=(64, 64),in_chans=1,patch_size=4,
                   window_size=8, img_range=1., depths=[4, 4, 4],
                   embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='')
    return model

# def swinIR_1_denoising(*kwargs):
#     model=SwinIR(upscale=1, img_size=(256, 256),in_chans=1,patch_size=8,
#                    window_size=64, img_range=1., depths=[4,4,4],
#                    embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='')
#     return model
#
# def swinIR_2_denoising(*kwargs):
#     model=SwinIR(upscale=1, img_size=(256, 256),in_chans=1,patch_size= 16,
#                    window_size=128, img_range=1., depths=[4,4,4],
#                    embed_dim=60, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='')
#     return model
